...
const stOrderSchema = new mongoose.Schema(
  {
    // 🔹 NEW: Link to the request that created this order
    requestId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "STRequest", // source of this order
      index: true
    },
    ...
    // 🔹 REMOVED: quantity, priceAtPurchase, totalAmount (now in request)
    // quantity and priceAtPurchase are now stored in the request document
...
// 🔹 UPDATED: Generate order number before validation (simpler format)
stOrderSchema.pre("validate", function (next) {
  if (!this.orderNumber) {
    const d = new Date();
